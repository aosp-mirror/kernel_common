/* drivers/i2c/chips/tps61310_flashlight_front.c
 *
...
#define FLT_DBG_LOG(fmt, ...) \
		printk(KERN_DEBUG "[FLT_FR]TPS " fmt, ##__VA_ARGS__)
#define FLT_INFO_LOG(fmt, ...) \
		printk(KERN_INFO "[FLT_FR]TPS " fmt, ##__VA_ARGS__)
#define FLT_ERR_LOG(fmt, ...) \
		printk(KERN_ERR "[FLT_FR][ERR]TPS " fmt, ##__VA_ARGS__)
...
struct delayed_work tps61310_delayed_work_front;
...
int tps61310_flashlight_control_front(int mode);
int tps61310_flashlight_mode_front(int mode);
		...
		const char *buf, size_t size)
{
	...
	if(val >= 0){
		FLT_INFO_LOG("%s: %d\n",__func__,val);
		tps61310_flashlight_mode_front(val);
	}else
		FLT_INFO_LOG("%s: Input out of range\n",__func__);
	return size;
}
...
void retry_flashlight_control_front(int err, int mode)
{
	if (err && !retry) {
		FLT_INFO_LOG("%s error once\n", __func__);
		retry++;
		mutex_unlock(&tps61310_mutex);
		tps61310_flashlight_control_front(mode);
...
int tps61310_flashlight_mode_front(int mode)
{
	...
	if (mode == 0)
		...
		queue_delayed_work(tps61310_work_queue, &tps61310_delayed_work_front,
...
int tps61310_flashlight_mode2_front(int mode2, int mode13)
{
	...
	if (mode2 == 0 && mode13 == 0)
		...
		if (mode13) {
			...
			tps61310_i2c_command(0x00, 0x00);
			tps61310_i2c_command(0x02, current_hex);
			FLT_INFO_LOG("set led13 current to 0x%x.\r\n", current_hex);
		}
		if (mode2) {
...
int tps61310_flashlight_torch_front(int led2, int led13)
{
	...
	uint8_t current_hex = 0x0;
	uint8_t current_hex2 = 0x0;
	uint8_t current_hex13 = 0x0;
	FLT_INFO_LOG("%s\n", __func__);
	FLT_INFO_LOG("front camera torch current =%d, %d \n", led2, led13);
	...
	if ( led2 == 0 && led13 ==0)
		...
		if (led2 == 0)
			...
			current_hex2 = (led2/ 25) & 0x07;
		if (led13 == 0)
			current_hex13 = 0x0;
		else if (led13 < 50)
			current_hex13 = 0x8;
		else
			current_hex13 = ((led13/ 50) << 3) & 0x38;
		current_hex = current_hex2 + current_hex13;
		FLT_INFO_LOG("torch current hex = %x\n", current_hex);
		tps61310_i2c_command(0x05, 0x6F);
...
int tps61310_flashlight_control_front(int mode)
{
	...
	if ( support_dual_flashlight ) {
			...
			case FL_MODE_TORCH:
				...
				if (this_tps61310->reset)
					retry_flashlight_control_front(err, mode);
			break;
						...
						enum led_brightness brightness)
{
	...
	retry = 0;
	ret = tps61310_flashlight_control_front(mode);
	if (ret) {
				...
				enum led_brightness value)
{
	...
	if (value < LED_OFF || value > led->cdev.max_brightness) {
		FLT_ERR_LOG("Invalid brightness value\n");
		return;
	}
					...
					const struct i2c_device_id *id)
{
	...
	htc_flashlight_flash		   = &tps61310_flashlight_mode2_front;
	htc_flashlight_torch		   = &tps61310_flashlight_torch_front;
